...
public class MainFrame extends JFrame {
    ...
    public Double calculate1(Double x, Double y,Double z) {
        return Math.pow(Math.cos(Math.pow(Math.E,x))+Math.log(Math.pow((1+y),2))+Math.pow(Math.pow(Math.E,Math.cos(x))+Math.pow(Math.sin(Math.PI*z),2),0.5)
                +Math.pow(1/x,0.5)+Math.cos(Math.pow(y,2)),Math.sin(z));
    }
...
// Создать область для кнопок
        ...
        JButton buttonMemoryPlus = new JButton("M+");
        textFieldM = new JTextField("0", 50);///1.03
...
// Связать области воедино в компоновке BoxLayout
        ...
        hboxMemory.add(Box.createHorizontalGlue());//1.05
